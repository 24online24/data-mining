...
def selectie(populatie, gen=None):
...
def g_a(dim_pop, nr_gen):
    ...
    for gen in range(nr_gen):
        ...
        if gen % 100 == 0:
            print(f"Generatie {gen} cea mai bună rută are distanța {populatie[0].fitness}")
        urm_pop = populatie[:2]
...
if __name__ == '__main__':
    ...
    best_traseu, dist = g_a(dim_pop=200, nr_gen=1000)
    ...
    print(best_traseu)
    print("-" * 75)
    print(f"{'Distance':>10} | {'Time':>10}")
    print("-" * 75)
    print(f"{dist:>10.2f} | {time:>10.2f}s")
